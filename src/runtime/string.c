...
__attribute__((visibility("default"))) const __type_info Stringˇtype = {
    .data_type =
        {
            .size = sizeof(String),
...
String¸fromCString(const char *value) {
  ...
  if (!data)
    panic("Out of memory in String¸fromCString");
...
__attribute__((visibility("default"))) String *String¸concat(String *left,
  ...
  char *data = malloc(len + 1);
  if (!data)
    panic("Out of memory in String¸concat");
...
__attribute__((visibility("default"))) truthvalue_t String¸eq(String *left,
                                                              String *right) {
  if (!left && !right)
    return c_true;
  if (!left || !right)
...
String¸readTextFile(String *path) {
  ...
  if (!data)
    panic("Out of memory in String¸readTextFile");
  ...
  return allocInitRC(String, 0, __rc_ISOLATED, .length = readCount,
                     .data = data);
}
...
String·toCString(String *self) {
  if (!self)
    panic("String·toCString does not accept NULL");
  return self->data;
}
