...
__attribute__((visibility("default"))) void *retainRC(void *const structure) {
  if (!structure)
    return NULL;

  atomic_fetch_add_explicit(&RC_HEADER(structure)->refs, 1,
...
__attribute__((visibility("default"))) void *
_release_rc_structure(void *const structure) {
  if (!structure)
    return NULL;
  __rc_header *const header = RC_HEADER(structure);

  const refs_t prevRefs =
      atomic_fetch_sub_explicit(&header->refs, 1, memory_order_acq_rel) &
      __rc_REFC_BITMASK;
  if (prevRefs == 1) {
    __rc_proxy *const proxy =
        atomic_load_explicit(&header->proxy, memory_order_acquire);
    if (proxy) {
...
__attribute__((visibility("default"))) void *shareRC(void *const structure) {
  __rc_header *const header = RC_HEADER(structure);
...
void *_performCopying(void *const structure, refs_t const semantics) {
  ...
  atomic_fetch_and_explicit(&header->refs, ~__rc_COPYING_FLAG,
                            memory_order_acquire);
  return clone;
}

__attribute__((visibility("default"))) void *_mutateRC(void *const structure) {
  __rc_header *const header = RC_HEADER(structure);
  // No copying needed if SHARED
  // But what if it is being copied (explicit copy)? It should not be possible
  // to change then, should it?
  if ((header->refs & __rc_SEMANTICS_FLAG) == __rc_SHARED)
    return structure;

  // Flag that copying is in progress.
  const refs_t refs = atomic_fetch_or_explicit(&header->refs, __rc_COPYING_FLAG,
                                               memory_order_acquire);
...
__attribute__((visibility("default"))) void *copyRC(void *const structure,
                                                    refs_t const semantics) {
  __rc_header *const header = RC_HEADER(structure);
  // Flag that copying is in progress.
  const refs_t refs = atomic_fetch_or_explicit(&header->refs, __rc_COPYING_FLAG,
                                               memory_order_acquire);
...
__attribute__((visibility("default"))) __rc_proxy *
retainWeakly(void *const structure) {
  if (!structure)
    return NULL;
  __rc_header *const header = RC_HEADER(structure);
  ...
  // Update header->proxy if its value is still NULL (the initial value of
  // raceProxy).
  __rc_proxy *raceProxy = NULL;
  if (atomic_compare_exchange_strong_explicit(&header->proxy, &raceProxy,
                                              newProxy, memory_order_acq_rel,
                                              memory_order_acquire)) {
...
__attribute__((visibility("default"))) void
_release_proxy(__rc_proxy *const proxy) {
