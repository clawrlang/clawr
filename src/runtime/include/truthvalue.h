...
static inline truthvalue_t adjust__towards(truthvalue_t value,
                                           truthvalue_t towards) {
  if (towards == c_true)
    return value == c_false ? c_ambiguous : c_true;
  if (towards == c_false)
    return value == c_true ? c_ambiguous : c_false;
