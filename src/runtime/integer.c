...
__attribute__((visibility("default"))) const __type_info Integerˇtype = {
    .data_type =
        {
            .size = sizeof(Integer),
...
__attribute__((visibility("default"))) Integer Integer¸zero = {
    .digits = &Array¸empty,
    .header = {
        .is_a = &Integerˇtype,
        .refs = 1,
...
void _add_in_place(Integer *const self, Integer *const addend,
                   ternary_digit_t sign);

__attribute__((visibility("default"))) void
Integer·increment(Integer *const self, Integer *const addend) {
  _add_in_place(self, addend, 1);
}

__attribute__((visibility("default"))) void
Integer·decrement(Integer *const self, Integer *const subtrahend) {
...
void expandWithZeros(Integer *const self, const int minCount) {
  if (minCount <= self->digits->count)
    return;

  Array *newDigitArray = Array¸new(minCount, sizeof(digit_t));
  for (int i = 0; i < self->digits->count; i++) {
    ARRAY_ELEMENT_AT(i, newDigitArray, digit_t) =
        ARRAY_ELEMENT_AT(i, self->digits, digit_t);
  }
...
void _add_in_place(Integer *const self, Integer *const subtrahend,
                   ternary_digit_t sign) {
  ...
  for (int i = 0; i < subtrahend->digits->count; i++) {
    AdditionResult addition_result =
        add(ARRAY_ELEMENT_AT(i, self->digits, digit_t),
            sign * ARRAY_ELEMENT_AT(i, subtrahend->digits, digit_t), carry);
    carry = addition_result.carry;
    ARRAY_ELEMENT_AT(i, self->digits, digit_t) = addition_result.digit;
  }

  for (int i = subtrahend->digits->count; carry != 0 && i < self->digits->count;
       i++) {
...
AdditionResult add(const digit_t a, const digit_t b,
                   const ternary_digit_t carry) {
  ...
  if (a > 0 && b > 0 && sum < 0) { // overflow
    ...
    return (AdditionResult){
        .carry = carry,
        .digit =
            sum + carry, // Move one extra step in the direction of the carry
    };
  } else {
    return (AdditionResult){
        .carry = 0,
...
Integer·toStringRC(Integer *self) {
...
Integer·toString(Integer *self) {
  return integerToCString(self);
}

__attribute__((visibility("default"))) void
Integer·toggleSign(Integer *const self) {
  mutateRC(self->digits);
...
Integer¸withDigits(Array *const digits) {
  ...
  for (size_t i = 0; i < digits->count; i++) {
    ARRAY_ELEMENT_AT(digits->count - 1 - i, integer->digits, digit_t) =
        ARRAY_ELEMENT_AT(i, digits, digit_t);
  }
...
__attribute__((visibility("default"))) void printDigits(Integer *integer) {
  printf("Digits: [ ");
...
__attribute__((visibility("default"))) digit_t Integer·divide(Integer *self,
                                                              digit_t divisor) {
  ...
  */
  ...
  for (size_t i = self->digits->count - 1; i > 0; i--) {
    ...
    remainder = remainder * BASE +
                (__int128_t)ARRAY_ELEMENT_AT(i, self->digits, digit_t);
  ...
  remainder =
      remainder * BASE + (__int128_t)ARRAY_ELEMENT_AT(0, self->digits, digit_t);
  ...
  if (ARRAY_ELEMENT_AT(self->digits->count - 1, self->digits, digit_t) == 0) {
    Array *truncatedDigits =
        Array¸new(self->digits->count - 1, sizeof(digit_t));
    for (size_t i = 0; i < truncatedDigits->count; i++) {
      ARRAY_ELEMENT_AT(i, truncatedDigits, digit_t) =
          ARRAY_ELEMENT_AT(i, self->digits, digit_t);
    }
...
static Integer *zeroOfSize(size_t count) {
  Integer *copy = allocInitRC(
      Integer, 0, __rc_ISOLATED,
      .digits =
          Array¸new(count, sizeof(digit_t)) // zero-initialised by Array¸new
  );
  return copy;
}
...
static Integer *copyInteger(Integer *src) {
  ...
  for (size_t i = 0; i < n; i++)
    ARRAY_ELEMENT_AT(i, copy->digits, digit_t) =
        ARRAY_ELEMENT_AT(i, src->digits, digit_t);
  return copy;
}
...
static void trimLeadingZeros(Integer *self) {
  ...
  for (size_t i = 0; i < n; i++)
    ARRAY_ELEMENT_AT(i, trimmed, digit_t) =
        ARRAY_ELEMENT_AT(i, self->digits, digit_t);
  releaseRC(self->digits);
  self->digits = trimmed;
}
...
static Integer *integerFromSingleDigit(digit_t value) {
  ...
  return allocInitRC(Integer, 0, __rc_ISOLATED, .digits = digits);
}
...
static void balancedDecompose(__int128_t value, digit_t *digit_out,
                              __int128_t *carry_out) {
  static const __int128_t BASE_128 = ((__int128_t)1 << 64) - 1;
  __int128_t q = value / BASE_128;
  __int128_t r = value - q * BASE_128;
  if (r > (digit_t)DIGIT_MAX) {
    r -= BASE_128;
    q++;
  } else if (r < -(digit_t)DIGIT_MAX) {
    r += BASE_128;
    q--;
  }
  *digit_out = (digit_t)r;
...
Integer¸multiply(Integer *left, Integer *right) {
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    for (size_t k = i + m; carry != 0 && k < n + m; k++) {
      __int128_t sum =
          (__int128_t)ARRAY_ELEMENT_AT(k, result->digits, digit_t) + carry;
      digit_t digit;
...
Integer¸divide(Integer *dividend, Integer *divisor) {
  ...
  // then parse back. This unblocks multi-limb division semantics needed by
  // Real.
...
__attribute__((visibility("default"))) int Integer¸compare(Integer *left,
                                                           Integer *right) {
...
__attribute__((visibility("default"))) truthvalue_t Integer¸eq(Integer *left,
                                                               Integer *right) {
  return Integer¸compare(left, right) == 0 ? c_true : c_false;
}

__attribute__((visibility("default"))) truthvalue_t Integer¸ne(Integer *left,
                                                               Integer *right) {
  return Integer¸compare(left, right) != 0 ? c_true : c_false;
}

__attribute__((visibility("default"))) truthvalue_t Integer¸lt(Integer *left,
                                                               Integer *right) {
  return Integer¸compare(left, right) < 0 ? c_true : c_false;
}

__attribute__((visibility("default"))) truthvalue_t Integer¸le(Integer *left,
                                                               Integer *right) {
  return Integer¸compare(left, right) <= 0 ? c_true : c_false;
}

__attribute__((visibility("default"))) truthvalue_t Integer¸gt(Integer *left,
                                                               Integer *right) {
  return Integer¸compare(left, right) > 0 ? c_true : c_false;
}

__attribute__((visibility("default"))) truthvalue_t Integer¸ge(Integer *left,
                                                               Integer *right) {
  return Integer¸compare(left, right) >= 0 ? c_true : c_false;
}
...
Integer¸fromStringRC(String *str) {
  ...
  while (*s == '0')
    ++s;
  if (!*s)
    return integerFromSingleDigit(0);
