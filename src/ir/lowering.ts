...
export function lowerASTtoIR(ast: ASTModule): CModule {
    return {
        body: ast.body.map(lowerStatement),
    }
}

function lowerStatement(stmt: ASTStatement): CStatement {
    if (stmt.kind === 'var-decl') {
        ...
        return lowerPrint(stmt)
    }
    throw new Error('Unknown AST statement kind')
}

function lowerValue(val: ASTExpression): CExpression {
...
function lowerPrint(print: ASTPrintStatement): CStatement {
            ...
            return {
                ...
                arguments: [
                    { kind: 'string', value: '%s\\n' },
                    {
                        ...
                        arguments: [
                            { kind: 'var-ref', name: `c_${print.value.value}` },
                        ],
                    },
                ],
            }
